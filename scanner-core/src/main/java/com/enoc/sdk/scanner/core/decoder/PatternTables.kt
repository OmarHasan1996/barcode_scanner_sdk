...
object PatternTables {
    ...
    private val R_CODE_BITS = L_CODE_BITS.map { bits -> bits.map { if (it == '1') '0' else '1' }.joinToString("") }
    private val G_CODE_BITS = R_CODE_BITS.map { it.reversed() }
    val FIRST_DIGIT_PARITY = arrayOf("LLLLLL", "LLGLGG", "LLGGLG", "LLGGGL", "LGLLGG", "LGGLLG", "LGGGLL", "LGLGLG", "LGLGGL", "LGGLGL")
    private fun bitsToRuns(bits: String): IntArray {
        val runs = ArrayList<Int>(); var current = bits[0]; var count = 1
        for (i in 1 until bits.length) { if (bits[i] == current) count++ else { runs.add(count); current = bits[i]; count = 1 } }
        runs.add(count); return runs.toIntArray()
    }
